...
void trdp_proxy_main(void *);
struct terminal * WinTerminalInit(HWND hWnd);
struct terminal * logterm;
				   ...
				   LPSTR lpCmdLine, int nCmdShow)
{
	...
	WNDCLASSEX wc;
	LPCTSTR MainWndClass = TEXT("ThinkOS RDP Proxy");
	HWND hWnd;
...

	{
		...
		HANDLE thread;
		void * arg = NULL;
		func = (unsigned (__stdcall *)(void *))trdp_proxy_main;
